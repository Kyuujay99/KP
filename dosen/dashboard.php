...
// Inisialisasi variabel untuk data summary dalam bentuk array, mirip admin
$stats = [
    'mahasiswa_bimbingan' => 0,
    'perlu_verifikasi' => 0,
    'perlu_nilai_dospem' => 0,
    'perlu_nilai_seminar' => 0
];
$total_perlu_tindakan = 0;
...
if ($conn && ($conn instanceof mysqli)) {
    ...
    if ($stmt_bimbingan) {
        ...
        if ($row = $result->fetch_assoc()) {
            $stats['mahasiswa_bimbingan'] = $row['total'];
        }
    ...
    if ($stmt_verifikasi) {
        ...
        if ($row = $result->fetch_assoc()) {
            $stats['perlu_verifikasi'] = $row['total'];
        }
    ...
    $stmt_nilai_dospem = $conn->prepare($sql_nilai_dospem);
     if ($stmt_nilai_dospem) {
        ...
        if ($row = $result->fetch_assoc()) {
            $stats['perlu_nilai_dospem'] = $row['total'];
        }
        $stmt_nilai_dospem->close();
    }

    // Hitung jumlah seminar yang perlu dinilai sebagai penguji
    $sql_nilai_seminar = "SELECT COUNT(sk.id_seminar) AS total FROM seminar_kp sk LEFT JOIN nilai_kp nk ON sk.id_pengajuan = nk.id_pengajuan WHERE (sk.nip_dosen_penguji1 = ? OR sk.nip_dosen_penguji2 = ?) AND sk.status_pelaksanaan_seminar = 'selesai' AND (nk.id_nilai IS NULL OR (sk.nip_dosen_penguji1 = ? AND nk.nilai_penguji1_seminar IS NULL) OR (sk.nip_dosen_penguji2 = ? AND nk.nilai_penguji2_seminar IS NULL))";
    $stmt_nilai_seminar = $conn->prepare($sql_nilai_seminar);
    if($stmt_nilai_seminar) {
        $stmt_nilai_seminar->bind_param("ssss", $nip_dosen_login, $nip_dosen_login, $nip_dosen_login, $nip_dosen_login);
        $stmt_nilai_seminar->execute();
        $result = $stmt_nilai_seminar->get_result();
        if ($row = $result->fetch_assoc()) {
            $stats['perlu_nilai_seminar'] = $row['total'];
        }
        $stmt_nilai_seminar->close();
    }

    // Hitung total item yang memerlukan tindakan
    $total_perlu_tindakan = $stats['perlu_verifikasi'] + $stats['perlu_nilai_dospem'] + $stats['perlu_nilai_seminar'];
}
...
<div class="dashboard-modern">
        ...
        <div class="hero-content">
            <div class="hero-avatar">
                <div class="avatar-circle">
                    <span class="avatar-text"><?php echo strtoupper(substr($nama_dosen_login, 0, 2)); ?></span>
                </div>
        ...
        <div class="analytics-grid">
            <div class="analytics-card card-students">
                ...
                <div class="card-content">
                    <div class="card-number" data-count="<?php echo $stats['mahasiswa_bimbingan']; ?>">0</div>
                    <div class="card-label">Mahasiswa Bimbingan</div>
            ...
            <div class="analytics-card card-pending <?php echo $stats['perlu_verifikasi'] > 0 ? 'alert-card' : ''; ?>">
                <div class="card-header">
                    <div class="card-icon">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    ...
                    <?php if ($stats['perlu_verifikasi'] > 0): ?>
                ...
                <div class="card-content">
                    <div class="card-number" data-count="<?php echo $stats['perlu_verifikasi']; ?>">0</div>
                    <div class="card-label">Pengajuan Perlu Verifikasi</div>
                    <?php if ($stats['perlu_verifikasi'] > 0): ?>
                        <a href="pengajuan_list.php" class="action-button pulse"><span>Lihat Detail</span><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg></a>
                    <?php endif; ?>
                </div>
            </div>

            <div class="analytics-card card-pending <?php echo $stats['perlu_nilai_dospem'] > 0 ? 'alert-card' : ''; ?>">
                <div class="card-header">
                    <div class="card-icon">
                         <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                     ...
                     <?php if ($stats['perlu_nilai_dospem'] > 0): ?>
                        <div class="card-trend negative"><span class="trend-icon">⚠</span><span class="trend-text">Urgent</span></div>
                    <?php else: ?>
                        <div class="card-trend neutral"><span class="trend-icon">✓</span><span class="trend-text">Clear</span></div>
                    <?php endif; ?>
                ...
                <div class="card-content">
                    <div class="card-number" data-count="<?php echo $stats['perlu_nilai_dospem']; ?>">0</div>
                    <div class="card-label">Perlu Penilaian Pembimbing</div>
                     <?php if ($stats['perlu_nilai_dospem'] > 0): ?>
                        <a href="nilai_input_list.php" class="action-button pulse"><span>Input Nilai</span><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg></a>
                    <?php endif; ?>
                </div>
            </div>

            <div class="analytics-card card-pending <?php echo $stats['perlu_nilai_seminar'] > 0 ? 'alert-card' : ''; ?>">
                <div class="card-header">
                    <div class="card-icon">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                     ...
                     <?php if ($stats['perlu_nilai_seminar'] > 0): ?>
                        <div class="card-trend negative"><span class="trend-icon">⚠</span><span class="trend-text">Urgent</span></div>
                    <?php else: ?>
                        <div class="card-trend neutral"><span class="trend-icon">✓</span><span class="trend-text">Clear</span></div>
                    <?php endif; ?>
                ...
                <div class="card-content">
                    <div class="card-number" data-count="<?php echo $stats['perlu_nilai_seminar']; ?>">0</div>
                    <div class="card-label">Perlu Penilaian Seminar</div>
                     <?php if ($stats['perlu_nilai_seminar'] > 0): ?>
                        <a href="seminar_jadwal_list.php" class="action-button pulse"><span>Beri Nilai</span><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg></a>
                    <?php endif; ?>
                </div>
            </div>
        </div>
    ...
    <div class="actions-section">
        <div class="section-header">
            <h2 class="section-title">Menu Manajemen Dosen</h2>
            <div class="section-line"></div>
        </div>

        <div class="actions-grid">
            <a href="bimbingan_mahasiswa_list.php" class="action-card" data-category="users">
                ...
                <div class="action-content">
                    ...
                    <div class="action-badge">Bimbingan</div>
                </div>
            ...
            <a href="pengajuan_list.php" class="action-card" data-category="monitoring">
                <div class="action-background"></div>
                <div class="action-icon">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14,2 14,8 20,8"/><path d="M12 18l-3-3 3-3"/><path d="M9 15h9"/>
                    </svg>
                </div>
                <div class="action-content">
                    <h3>Verifikasi Pengajuan KP</h3>
                    <p>Setujui atau tolak pengajuan KP dari mahasiswa bimbingan.</p>
                    <div class="action-badge">Verifikasi</div>
                </div>
                <div class="action-arrow"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg></div>
            </a>

            <a href="nilai_input_list.php" class="action-card" data-category="reports">
                <div class="action-background"></div>
                <div class="action-icon">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="M12 20V10"/><path d="M18 20V4"/><path d="M6 20v-4"/>
                ...
                <div class="action-content">
                    <h3>Input & Kelola Nilai</h3>
                    <p>Berikan penilaian akhir sebagai dosen pembimbing.</p>
                    <div class="action-badge">Akademik</div>
                </div>
                <div class="action-arrow"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg></div>
            </a>

            <a href="seminar_jadwal_list.php" class="action-card" data-category="documents">
                <div class="action-background"></div>
                <div class="action-icon">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                ...
                <div class="action-content">
                    <h3>Jadwal & Penilaian Seminar</h3>
                    <p>Lihat jadwal seminar dan berikan nilai sebagai penguji.</p>
                    <div class="action-badge">Seminar</div>
                </div>
                <div class="action-arrow"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg></div>
            </a>
            ...
            <a href="profil.php" class="action-card" data-category="users">
                ...
                <div class="action-content">
                    ...
                    <div class="action-badge">Manajemen Akun</div>
                </div>
                <div class="action-arrow"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg></div>
            </a>
        </div>
...
// Menutup koneksi database yang dibuka di awal
