<?php
// /KP/index.php (Versi Final dengan Perbaikan Logika & Keamanan)

if (session_status() == PHP_SESSION_NONE) {
    session_start();
}

// 1. Jika pengguna sudah login, langsung tendang ke dashboard-nya
...
// 2. Proses Login kalau ada yang neken tombol submit
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Peta sakti buat nentuin tabel mana yang dicek
    $table_map = [
        "mahasiswa"   => ['table' => "mahasiswa",       'id_col' => "nim",          'name_col' => "nama",       'pass_col' => "password",            'id_field' => "nim",               'status_col' => 'status_akun', 'redirect_path' => '/KP/mahasiswa/dashboard.php'],
        "dosen"       => ['table' => "dosen_pembimbing", 'id_col' => "nip",          'name_col' => "nama_dosen", 'pass_col' => "password",            'id_field' => "nip",               'status_col' => 'status_akun', 'redirect_path' => '/KP/dosen/dashboard.php'],
        "admin_prodi" => ['table' => "admin_prodi",     'id_col' => "id_admin",     'name_col' => "nama_admin", 'pass_col' => "password",            'id_field' => "username",          'redirect_path' => '/KP/admin_prodi/dashboard.php'],
        "perusahaan"  => ['table' => "perusahaan",      'id_col' => "id_perusahaan",'name_col' => "nama_perusahaan",'pass_col' => "password_perusahaan",'id_field' => "email_perusahaan",'status_col' => 'status_akun', 'redirect_path' => '/KP/perusahaan/dashboard.php']
    ];
    ...
    if (array_key_exists($role, $table_map)) {
        ...
        // Bikin query yang fleksibel
        $sql = "SELECT * FROM {$config['table']} WHERE $id_field = ? LIMIT 1";
        if ($role === 'mahasiswa' || $role === 'dosen') {
             $sql = "SELECT * FROM {$config['table']} WHERE ($id_field = ? OR email = ?) LIMIT 1";
        }
        ...
        if ($stmt) {
            ...
            if ($result->num_rows == 1) {
                ...
                // 1. Cek password dulu
                if ($password_input === $user[$config['pass_col']]) {
                    
                    // 2. Kalau password bener, baru cek status akun
                    if (isset($config['status_col'])) {
                        if ($user[$config['status_col']] === 'pending_verification' || $user[$config['status_col']] === 'pending_approval') {
                            $error_message = "Akun kamu lagi nunggu verifikasi Admin, sabar yaa, bub.";
                        } elseif ($user[$config['status_col']] === 'suspended' || $user[$config['status_col']] === 'inactive') {
                             $error_message = "Yah, akun kamu lagi nonaktif. Coba hubungi admin, ya.";
                        } else {
                            // Kalau status 'active', baru boleh masuk
                            $_SESSION['user_id'] = $user[$config['id_col']];
                            $_SESSION['user_nama'] = $user[$config['name_col']];
                            $_SESSION['user_role'] = $role;
                            header("Location: " . $config['redirect_path']); // Redirect ke dashboard yang bener
                            exit();
                        }
                    } else {
                        // Khusus buat admin yang nggak punya status
                        $_SESSION['user_id'] = $user[$config['id_col']];
                        $_SESSION['user_nama'] = $user[$config['name_col']];
                        $_SESSION['user_role'] = $role;
                        header("Location: " . $config['redirect_path']);
                        exit();
                    }
                } else {
                    $error_message = "Duh, passwordnya salah, ayang. Coba lagi deh.";
                }
            } else {
                $error_message = "User ID atau Email nggak ketemu, bub. Yakin udah bener ngetiknya?";
            }
            $stmt->close();
        }
    } else {
        $error_message = "Kamu milih peran apaan tuh? Nggak ada di daftar, hehe.";
    }
...
<head>
    ...
    <style>
        ...
        body { font-family: 'Poppins', sans-serif; margin: 0; height: 100vh; overflow: hidden; display: flex; justify-content: center; align-items: center; background: linear-gradient(-45deg,  #c3cfe2, #2c3e50, #2d3748, #0f172a); background-size: 400% 400%; animation: gradientBG 18s ease infinite; }
        ...
        .register-link-container { text-align: center; margin-top: 1.5rem; color: #fff; }
