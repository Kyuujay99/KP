<?php
// /KP/perusahaan/pengajuan_kp_konfirmasi.php (Versi Modern & Terisolasi)
...
function getPengajuanDetailsForCompany($conn_db, $pengajuan_id, $id_perusahaan, &$out_error_message) {
    ...
    if (!$conn_db) { $out_error_message = "Koneksi DB tidak valid."; return $data; }
    ...
    if ($stmt_pengajuan) {
        ...
        if ($result_pengajuan->num_rows === 1) {
            ...
            // Ambil juga dokumennya
            $sql_dokumen = "SELECT nama_dokumen, jenis_dokumen, file_path FROM dokumen_kp WHERE id_pengajuan = ? AND tipe_uploader = 'mahasiswa'";
            ...
            if ($stmt_dokumen) {
                ...
                $data['dokumen'] = $result_dokumen->fetch_all(MYSQLI_ASSOC);
                $stmt_dokumen->close();
            }
        } else {
             $out_error_message = "Pengajuan KP tidak ditemukan atau statusnya tidak lagi menunggu konfirmasi dari Anda.";
        }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_konfirmasi']) && $id_pengajuan_url) {
    ...
    $new_status_kp = ($tindakan_konfirmasi === 'terima') ? 'diterima_perusahaan' : 'ditolak_perusahaan';
    
    $surat_balasan_path = null;
    if (isset($_FILES["surat_balasan_perusahaan"]) && $_FILES["surat_balasan_perusahaan"]["error"] == UPLOAD_ERR_OK) {
        ...
        $unique_filename = "surat_balasan_" . $id_pengajuan_url . "_" . time() . "." . $file_ext;
        ...
        if (in_array($file_ext, $allowed_types) && $_FILES["surat_balasan_perusahaan"]["size"] <= 5000000) {
            if (move_uploaded_file($_FILES["surat_balasan_perusahaan"]["tmp_name"], $target_file)) {
                $surat_balasan_path = $target_file;
            } else {
                $error_message = "Gagal mengupload file.";
            }
        } else {
            $error_message = "Format atau ukuran file tidak sesuai.";
        }
    }
    ...
    if(empty($error_message)) {
        ...
        try {
            $stmt_update = $conn->prepare("UPDATE pengajuan_kp SET status_pengajuan = ?, surat_balasan_perusahaan_path = ? WHERE id_pengajuan = ? AND id_perusahaan = ?");
            $stmt_update->bind_param("ssii", $new_status_kp, $surat_balasan_path, $id_pengajuan_url, $id_perusahaan_login);
            $stmt_update->execute();
            
            if ($stmt_update->affected_rows === 0) {
                throw new Exception("Gagal memperbarui status. Mungkin status sudah diubah.");
            }
            $stmt_update->close();

            if($surat_balasan_path !== null) {
                ...
                $stmt_doc->bind_param("issss", $id_pengajuan_url, $uploader_id_str, $nama_dok, $jenis_dok, $surat_balasan_path);
                $stmt_doc->execute();
                $stmt_doc->close();
            }
            
            $conn->commit();
            $success_message = "Konfirmasi telah berhasil disimpan. Status pengajuan diperbarui.";
            header("refresh:3;url=" . $_SERVER['REQUEST_URI']);

        } catch (Exception $e) {
            $conn->rollback();
            $error_message = "Terjadi kesalahan: " . $e->getMessage();
            if ($surat_balasan_path && file_exists($surat_balasan_path)) { unlink($surat_balasan_path); }
        }
    }
}

// Ambil data terbaru untuk ditampilkan
if ($id_pengajuan_url && empty($error_message_initial_load)) {
    $fetch_error_temp = '';
...
<div class="kp-konfirmasi-modern-container">
    <div class="form-hero-section">
        <div class="form-hero-content">
            <div class="form-hero-icon"><svg viewBox="0 0 24 24"><path d="M14 9V5a3 3 0 0 0-3-3l-4 9v11h11.28a2 2 0 0 0 2-1.7l1.38-9a2 2 0 0 0-2-2.3zM7 22H4a2 2 0 0 1-2-2v-7a2 2 0 0 1 2-2h3"></path></svg></div>
        ...
        <?php if ($pengajuan_detail): ?>
            ...
            <form action="<?php echo htmlspecialchars($_SERVER['REQUEST_URI']); ?>" method="POST" enctype="multipart/form-data" class="modern-form animate-on-scroll">
                <div class="form-step">
                    ...
                    <div class="form-actions"><button type="submit" name="submit_konfirmasi" class="btn-submit"><svg viewBox="0 0 24 24"><line x1="22" y1="2" x2="11" y2="13"></line><polygon points="22 2 15 22 11 13 2 9 22 2"></polygon></svg>Kirim Konfirmasi</button></div>
                </div>
            </form>
            <?php endif; ?>
        <?php endif; ?>
