...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($role) || empty($email) || empty($password)) {
        $registration_message = "Harap lengkapi semua field yang diperlukan.";
        ...
        if ($role == "mahasiswa") {
            ...
            if ($stmt_check->num_rows > 0) {
                ...
                // Gunakan password dari form, BUKAN hash, sesuai permintaan
...
<head>
    ...
    <style>
        ...
        body { font-family: 'Poppins', sans-serif; margin: 0; min-height: 100vh; overflow-x: hidden; display: flex; justify-content: center; align-items: center; padding: 2rem 1rem; background: linear-gradient(-45deg, #6a11cb, #2575fc, #ec008c, #fc6767); background-size: 400% 400%; animation: gradientBG 18s ease infinite; }
        ...
        .message { padding: 1rem; margin-bottom: 1.5rem; border-radius: 8px; text-align: center; font-size: 0.9rem;}
        ...
        .login-link-container { text-align: center; margin-top: 1.5rem; color: #fff; } /* Diubah */
        .login-link { color: #fff; font-weight: 600; text-decoration: none; transition: all 0.3s ease; }
        .login-link:hover { color: #ffd700; text-shadow: 0 0 10px rgba(255, 215, 0, 0.5); } /* Diubah */
        ...
        <form action="/KP/register.php" method="POST" id="registrationForm">
            ...
            <div id="auth_fields" class="form-section">
                <div class="input-group">
                    <input type="email" id="email" name="email" placeholder=" "><label for="email">Email (*)</label>
                </div>
                <div class="input-group">
                    <input type="password" id="password" name="password" minlength="6" placeholder=" "><label for="password">Password (*)</label>
                </div>
                <div class="input-group">
                    <input type="password" id="confirm_password" name="confirm_password" minlength="6" placeholder=" "><label for="confirm_password">Konfirmasi Password (*)</label>
                </div>
            </div>

            <!-- Form Mahasiswa -->
            <div id="mahasiswa_fields" class="form-section">
                <div class="input-group"><input type="text" id="nim" name="nim" placeholder=" "><label for="nim">NIM (*)</label></div>
                <div class="input-group"><input type="text" id="nama_mahasiswa" name="nama_mahasiswa" placeholder=" "><label for="nama_mahasiswa">Nama Lengkap (*)</label></div>
                <div class="input-group custom-select"><select id="prodi" name="prodi"><option value="" disabled selected></option><option value="Teknik Informatika">Teknik Informatika</option><option value="Sistem Informasi">Sistem Informasi</option><option value="Teknik Elektro">Teknik Elektro</option><option value="Teknik Mesin">Teknik Mesin</option></select><label for="prodi">Program Studi (*)</label></div>
                <div class="input-group"><input type="number" id="angkatan" name="angkatan" min="2000" max="<?php echo date('Y'); ?>" placeholder=" "><label for="angkatan">Angkatan (*)</label></div>
                <div class="input-group"><input type="text" id="no_hp_mahasiswa" name="no_hp_mahasiswa" placeholder=" "><label for="no_hp_mahasiswa">No. HP (Opsional)</label></div>
            </div>

            <!-- Form Dosen -->
            <div id="dosen_fields" class="form-section">
                <div class="input-group"><input type="text" id="nip" name="nip" placeholder=" "><label for="nip">NIP (*)</label></div>
                <div class="input-group"><input type="text" id="nama_dosen" name="nama_dosen" placeholder=" "><label for="nama_dosen">Nama Lengkap & Gelar (*)</label></div>
            </div>

            <!-- Form Perusahaan -->
            <div id="perusahaan_fields" class="form-section">
                <div class="input-group"><input type="text" id="nama_perusahaan" name="nama_perusahaan" placeholder=" "><label for="nama_perusahaan">Nama Perusahaan (*)</label></div>
                <div class="input-group"><textarea id="alamat_perusahaan" name="alamat_perusahaan" rows="2" placeholder=" "></textarea><label for="alamat_perusahaan">Alamat (Opsional)</label></div>
                <div class="input-group"><input type="text" id="bidang_perusahaan" name="bidang_perusahaan" placeholder=" "><label for="bidang_perusahaan">Bidang Usaha (Opsional)</label></div>
            </div>
        ...
        function toggleRoleFields() {
            ...
            if (role) {
                ...
                if (sectionToShow) {
                    ...
                    sectionToShow.querySelectorAll('input, select').forEach(input => {
